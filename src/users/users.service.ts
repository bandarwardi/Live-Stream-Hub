...
export class UsersService {
  ...
  async findByFirebaseUid(firebaseUid: string): Promise<User | null> {
    return this.userModel
      .findOne({ firebaseUid, isDeleted: { $ne: true } })
      .exec();
  }
  ...
  async findOrCreateByFirebaseUid(
    ...
    if (!defaults.username) {
      ...
      defaults.username = `user_${randomString}`;
    }
    return this.userModel
      .findOneAndUpdate(
        { firebaseUid: uid },
  ...
  async updateProfile(userId: string, data: Partial<User>) {
    try {
      ...
      const merged = { ...user.toObject(), ...data };
      if (merged.displayName && merged.gender && merged.birthdate) {
      ...
      if (err.code === 11000) {
        ...
        throw new ConflictException(
          `هذا ال${field} مستخدم بالفعل من قبل حساب آخر`,
        );
      }
      throw err;
    }
